...
# Directories
PROJECT_DIR="$HOME/k8s-utm-ha-homelab"
...
if [[ ! -f "$SSH_KEY_PRIVATE" ]] || [[ ! -f "$SSH_KEY_FILE" ]]; then
    ...
    ssh-keygen -t ed25519 -f "$SSH_KEY_PRIVATE" -N "" -C "k8s-utm-ha-homelab" -q
...
download_binaries() {
    ...
    # Helper: download a file in background if not cached
    bg_download() {
...
# Note: jump cloud-init is still installing ansible/vault/terraform in background.
# We don't wait here - tools aren't needed until Step 12 (Vault Full Setup).
# Steps 10, 10.5, 11 only use SSH and scp which are already available.
...
if [[ "$JUMP_READY" == "true" ]]; then
    ...
    echo -n "  Creating ~/k8s-utm-ha-homelab on jump..."
    ssh jump "mkdir -p ~/k8s-utm-ha-homelab" 2>/dev/null && echo -e " ${GREEN}OK${NC}" || echo -e " ${RED}FAILED${NC}"

    echo -n "  Copying ansible/..."
    scp -r "$PROJECT_DIR/ansible" jump:~/k8s-utm-ha-homelab/ 2>/dev/null && echo -e " ${GREEN}OK${NC}" || echo -e " ${RED}FAILED${NC}"

    echo -n "  Copying execution_flow.txt..."
    scp "$PROJECT_DIR/execution_flow.txt" jump:~/k8s-utm-ha-homelab/ 2>/dev/null && echo -e " ${GREEN}OK${NC}" || echo -e " ${YELLOW}SKIP${NC}"

    echo -e "${GREEN}✓${NC} Project files copied to jump:~/k8s-utm-ha-homelab/"
else
...
export VAULT_TOKEN=$(jq -r .root_token ~/k8s-utm-ha-homelab/ansible/.vault-credentials/vault-init.json 2>/dev/null)

# Unseal vault after vault server reboot (uses first 3 keys)
vault-unseal() {
    echo "Unsealing Vault..."
    local creds="$HOME/k8s-utm-ha-homelab/ansible/.vault-credentials/vault-init.json"
    if [[ ! -f "$creds" ]]; then
...
if [[ "$JUMP_OK" == "true" ]]; then
    ...
    if ssh jump 'which ansible-playbook' &>/dev/null; then
        ...
        if ssh jump 'cd ~/k8s-utm-ha-homelab/ansible && ansible-playbook -i inventory/ playbooks/vault-full-setup.yml && touch ~/.vault-bootstrapped'; then
            echo ""
            ...
            if ssh jump 'cd ~/k8s-utm-ha-homelab/ansible && ansible-playbook -i inventory/ playbooks/k8s-certs.yml'; then
                echo ""
                ...
                ssh jump 'cd ~/k8s-utm-ha-homelab/ansible && ansible-playbook -i inventory/ playbooks/etcd-cluster.yml' &
                ETCD_PID=$!
                
                # Run HAProxy in background
                ssh jump 'cd ~/k8s-utm-ha-homelab/ansible && ansible-playbook -i inventory/ playbooks/haproxy.yml' &
                HAPROXY_PID=$!
                ...
                if [[ "$ETCD_OK" == "true" ]]; then
                    ...
                    echo "  ssh jump 'cd ~/k8s-utm-ha-homelab/ansible && ansible-playbook -i inventory/ playbooks/etcd-cluster.yml'"
                fi
                ...
                if [[ "$HAPROXY_OK" == "true" ]]; then
                    ...
                    echo "  ssh jump 'cd ~/k8s-utm-ha-homelab/ansible && ansible-playbook -i inventory/ playbooks/haproxy.yml'"
                fi
                
                if [[ "$ETCD_OK" == "true" ]]; then

                    # Step 14a.5: Store etcd encryption key in Vault
                    header "Step 14a.5/17: Store etcd encryption key in Vault"
                    if ssh jump 'cd ~/k8s-utm-ha-homelab/ansible && ansible-playbook -i inventory/ playbooks/vault-etcd-encryption-key.yml'; then
                        echo ""
                        echo -e "${GREEN}✓${NC} etcd encryption key stored in Vault!"
                    else
                        echo -e "${RED}✗${NC} Failed to store etcd encryption key - run manually:"
                        echo "  ssh jump 'cd ~/k8s-utm-ha-homelab/ansible && ansible-playbook -i inventory/ playbooks/vault-etcd-encryption-key.yml'"
                    fi
                    ...
                    if ssh jump 'cd ~/k8s-utm-ha-homelab/ansible && ansible-playbook -i inventory/ playbooks/control-plane.yml'; then
                        echo ""
                        ...
                        if ssh jump 'cd ~/k8s-utm-ha-homelab/ansible && ansible-playbook -i inventory/ playbooks/worker.yml'; then
                            echo ""
                            ...
                            echo "  ssh jump"
                            echo "  cd ~/k8s-utm-ha-homelab/ansible"
