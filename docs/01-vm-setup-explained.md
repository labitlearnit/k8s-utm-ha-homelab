...
This document explains how we automated the creation of 11 Ubuntu VMs on Apple Silicon Mac using UTM virtualization for a production-like Kubernetes cluster.
...
The `arm64` suffix is critical for Apple Silicon (M1/M2/M3/M4 chips).
...
.12         Jump (Bastion / Ansible Controller)
...
The `k8s-utm-ha-homelab.sh` script performs the full end-to-end deployment in 17 steps:

```
┌──────────────────────────────────────────────────────────────┐
│                  k8s-utm-ha-homelab.sh                        │
│                                                              │
│  Infrastructure Setup (Mac)                                  │
│  ─────────────────────────                                   │
│  Step 1:  Download Ubuntu 24.04 cloud image                 │
│  Step 2:  Generate SSH key (~/.ssh/k8slab.key)              │
│  Step 2.5: Download K8s binaries in background              │
│  Step 3:  Create 11 VMs (cloud-init ISOs + config.plist)    │
│  Step 4:  Restart UTM to detect new VMs                     │
│  Step 5:  Update Mac /etc/hosts                             │
│  Step 6:  Setup SSH config (bastion via jump)               │
│  Step 7:  Start all VMs                                     │
│  Step 8:  Wait for VMs to boot                              │
│  Step 9:  Configure jump server (SSH keys, project files)   │
│  Step 10: Connectivity test (via jump)                      │
│  Step 10.5: Wait for background binary downloads            │
│  Step 11: Setup Vault environment on jump                   │
│                                                              │
│  Cluster Deployment (from jump)                              │
│  ──────────────────────────────                              │
│  Step 12: Vault full setup (bootstrap + PKI)                │
│  Step 13: Deploy K8s certificates                           │
│  Step 14: Deploy etcd cluster + HAProxy (parallel)          │
│  Step 15: Deploy control plane (2 masters)                  │
│  Step 16: Deploy worker nodes (3 workers)                   │
│  Step 17: Install Calico CNI                                │
│                                                              │
└──────────────────────────────────────────────────────────────┘
```

An equivalent Ansible playbook (`k8s-utm-ha-homelab.yml`) performs the same deployment with the same parallelism strategy.
...
├── scripts/
│   └── k8s-utm-ha-homelab.sh             # Main automation script
└── docs/
...
**Result:** 11 production-like VMs created and a full Kubernetes cluster deployed, fully automated, with static IPs, SSH access, Vault-managed PKI, and hostname resolution.
